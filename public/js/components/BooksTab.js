...
Vue.component("books-tab", {
    ...
    methods: {
            ...
            try {
                ...
                if (result.error) {
                    ...
                    if (book) {
                        ...
                        book.sync_to_onenote = !enabled ? 1 : 0;
                    }
                    this.filterBooks();
                    return;
                }
                ...
                if (enabled) {
                    ...
                    this.showStatus("已停用 Joplin 同步", "success");
                }
            } catch (error) {
        ...
        async mergeBooksWithPlus() {
            if (
                !confirm(
                    "確定要合併所有以 '+' 結尾的書籍嗎？此操作會將這些書籍的章節合併到對應的基礎書籍中，並刪除以 '+' 結尾的書籍。"
                )
            ) {
                return;
            }
            ...
            this.merging = true;
            try {
                const result = await window.API.mergeBooksWithPlus();
                if (result.error) {
                    this.showStatus("合併失敗: " + result.error, "error");
                    return;
                }

                let message =
                    result.message ||
                    `合併完成: ${result.merged || 0} 個章節已合併，${
                        result.deleted || 0
                    } 本書籍已刪除`;
                ...
                if (result.results && result.results.length > 0) {
                    ...
                    if (errorCount > 0 || skippedCount > 0) {
                        message += `\n成功: ${successCount}, 跳過: ${skippedCount}, 錯誤: ${errorCount}`;
                    }
                }
                ...
                this.showStatus(
                    "合併時發生錯誤: " + (error.message || "Unknown error"),
                    "error"
                );
            } finally {
                this.merging = false;
