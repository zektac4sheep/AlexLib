    ...
    methods: {
        ...
        async analyzeAndShowModal() {
            ...
            try {
                ...
                // Log chapter numbers and series to console
                if (analysis.chapters && analysis.chapters.length > 0) {
                    console.log('Parsed chapters:', analysis.chapters.map(ch => ({ 
                        number: ch.number, 
                        series: ch.series || 'official' 
                    })));
                    analysis.chapters.forEach(ch => {
                        console.log(`Chapter ${ch.number} - Series: ${ch.series || 'official'}`);
                    });
                } else {
                    console.log('No chapters found in parsed file');
                }
