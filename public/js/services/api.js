...
window.API = {
    ...
    async deleteChapter(bookId, chapterId) {
        ...
        return handleResponse(response, "刪除章節失敗");
    },
    ...
    async mergeBooksWithPlus() {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/books/merge-books-with-plus`,
            {
                method: "POST",
            },
            120000
        ); // Longer timeout for merge operations
        return handleResponse(response, "合併書籍失敗");
    },

    async removeEbooksFolder(port, token) {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/joplin/remove-ebooks-folder`,
            {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            ...
            300000
        ); // Longer timeout for removing Ebooks folder
        return handleResponse(response, "移除 Ebooks 資料夾失敗");
    },
    ...
    async forceSyncJoplinBooks(port, token) {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/joplin/force-sync-books`,
            {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ port, token }),
        ...
        return handleResponse(response, "強制同步書籍失敗");
    },
    ...
    // Source Joplin API
    async getJoplinSettings() {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/joplin/settings`
        );
        return handleResponse(response, "載入 Joplin 設定失敗");
    },

    async saveJoplinSettings(settings) {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/joplin/settings`,
            {
                method: "POST",
        ...
        return handleResponse(response, "儲存 Joplin 設定失敗");
    },

    // OneNote API
    async testOneNoteConnection() {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/joplin/test-connection`,
            {
                method: "POST",
    ...
    async getOneNoteStructure() {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/joplin/onenote/structure`
        );
        return handleResponse(response, "載入 OneNote 結構失敗");
    },

    async getOneNoteNotebooks() {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/joplin/onenote/notebooks`
        );
        return handleResponse(response, "載入 OneNote 筆記本失敗");
    },

    async syncOneNoteBooks() {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/joplin/sync-books`,
            {
                method: "POST",
    ...
    async recreateOneNoteBookSection(bookId) {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/joplin/recreate-book-folder/${bookId}`,
            {
                method: "POST",
        ...
        return handleResponse(response, "重新建立書籍區段失敗");
    },
    ...
    async getSourceJoplinNotesByFolder(folderId) {
        ...
        const response = await fetchWithTimeout(
            `${API_BASE}/api/joplin/source/notes?folderId=${encodeURIComponent(
                folderIdParam
            )}`
        );
    ...
    async debugSourceJoplinFolder(folderName) {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/joplin/source/debug/folder/${encodeURIComponent(
                folderName
            )}`
        );
    ...
    async getSourceJoplinFolderNoteCount(folderId) {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/joplin/source/folders/${encodeURIComponent(
                folderId
            )}/note-count`
        );
    ...
    async searchSourceJoplinNotes(subtext, limit = 100) {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/joplin/source/search?subtext=${encodeURIComponent(
                subtext
            )}&limit=${limit}`,
            {},
            60000
        );
        return handleResponse(response, "搜尋筆記失敗");
    },

    async removeSourceEbooksFolder() {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/joplin/source/remove-ebooks-folder`,
            {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            },
    ...
    async getBooksNeedingChunks() {
        const response = await fetchWithTimeout(`${API_BASE}/api/chunks/books-needing-chunks`);
        return handleResponse(response, "載入需要分塊的書籍列表失敗");
    },
    ...
    async rebuildAllChunks(chunkSize = 1000) {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/chunks/rebuild-all`,
            {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ chunkSize }),
        ...
        return handleResponse(response, "重建所有分塊失敗");
    },

    async reformatChunk(bookId, chunkNumber) {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/chunks/books/${bookId}/chunks/${chunkNumber}/reformat`,
            {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            },
        ...
        return handleResponse(response, "重新格式化分塊失敗");
    },

    async reformatBookChunks(bookId, chunkSize = 1000) {
        const response = await fetchWithTimeout(
            `${API_BASE}/api/chunks/books/${bookId}/reformat`,
            {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ chunkSize }),
        ...
        return handleResponse(response, "重新格式化書籍分塊失敗");
    },
