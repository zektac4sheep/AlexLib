...
class Book {
  static async create(bookNameSimplified, bookNameTraditional = null, metadata = {}) {
  ...
  static async findBySimplifiedName(bookNameSimplified) {
    const db = getDatabase();
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM books WHERE book_name_simplified = ?', [bookNameSimplified], (err, row) => {
        if (err) {
          reject(err);
        } else {
          resolve(row);
  ...
  static async findAll() {
    const db = getDatabase();
    return new Promise((resolve, reject) => {
      db.all('SELECT * FROM books ORDER BY last_updated DESC', [], (err, rows) => {
        if (err) {
          reject(err);
        } else {
          resolve(rows);
          ...
          if (updates.tags !== undefined) {
            ...
            db.run('DELETE FROM book_tags WHERE book_id = ?', [id], () => {
              ...
              if (updates.tags.length > 0) {
                Book.addTags(id, updates.tags).catch(err => {
