...
class Chapter {
    ...
    return new Promise((resolve, reject) => {
      const sql = `
        INSERT INTO chapters (
          book_id, chapter_number, chapter_title, chapter_title_simplified,
      ...
      db.run(sql, [
        book_id, chapter_number, chapter_title, chapter_title_simplified,
        cool18_url, cool18_thread_id, content, line_start, line_end, status
      ], function(err) {
        if (err) {
          // If duplicate, try to update instead
          if (err.message.includes('UNIQUE constraint')) {
            Chapter.updateByBookAndNumber(book_id, chapter_number, chapterData)
              .then(resolve)
              .catch(reject);
          } else {
            reject(err);
  ...
  static async updateByBookAndNumber(bookId, chapterNumber, updates) {
    ...
    const sql = `UPDATE chapters SET ${fields.join(', ')} WHERE book_id = ? AND chapter_number = ?`;
  ...
  static async findByUrl(cool18Url) {
    const db = getDatabase();
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM chapters WHERE cool18_url = ?', [cool18Url], (err, row) => {
        if (err) {
          reject(err);
        } else {
          resolve(row);
