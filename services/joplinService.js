 ...
 * @returns {Promise<void>}
 */
async function updateNote(noteId, title = null, body = null, parentId = null, options = {}) {
    try {
        const updateData = {};
...
async function deleteNote(noteId, options = {}) {
    logger.warn("deleteNote is deprecated, using moveNoteToRecyclePool instead", {
        noteId,
    });
...
async function deleteAllNotesInNotebook(notebookId, options = {}) {
    logger.warn("deleteAllNotesInNotebook is deprecated, using moveAllNotesToRecyclePool instead", {
        notebookId,
    });
    ...
    try {
        ...
        if (notes && notes.length > 0) {
            for (const note of notes) {
                try {
                    await updateNote(note.id, null, null, recyclePoolId, options);
                    movedCount++;
                } catch (moveError) {
...
async function syncChunksToJoplin(
    ...
    try {
        ...
        const needsSync =
            ...
            new Date(chunkJob.completed_at) > new Date(book.last_synced_to_joplin);
        ...
        for (const dbChunk of allBookChunks) {
            if (dbChunk.joplin_note_id && !currentChunkIds.has(dbChunk.id)) {
                try {
                    await moveNoteToRecyclePool(dbChunk.joplin_note_id, options);
                    ...
                    logger.warn("Error moving orphaned chunk note to recycle pool", {
                        bookId: book.id,
                        chunkId: dbChunk.id,
                        noteId: dbChunk.joplin_note_id,
                        error: moveError.message,
    ...
    try {
        ...
        if (book.joplin_notebook_id) {
            try {
                ...
                logger.info("Moved notes to recycle pool before recreating folder", {
                    notebookId: book.joplin_notebook_id,
                    movedCount,
                });
            } catch (moveError) {
                logger.warn("Error moving notes to recycle pool (may not exist)", {
                    notebookId: book.joplin_notebook_id,
                    error: moveError.message,
                });
            }
